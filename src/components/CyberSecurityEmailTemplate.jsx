...
import { Copy, Download, Eye, ShieldCheck, Mail, Send } from 'lucide-react';
...
const CyberSecurityEmailTemplate = () => {
  ...
  const [emailSubject, setEmailSubject] = useState('🛡️ Fortify Your Enterprise: Elite Cybersecurity Staffing Solutions');
  ...
  return (
    <div className="container mx-auto max-w-7xl px-2 sm:px-4 py-4 sm:py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="space-y-4 sm:space-y-8">
        {/* UI Controls */}
        <div className="bg-slate-950 dark:bg-slate-900 rounded-xl shadow-2xl border border-cyan-900/50 dark:border-cyan-700/50 p-4 sm:p-6 transition-colors">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4 w-full md:w-auto">
              <div className="p-2 sm:p-3 bg-cyan-500/10 rounded-lg border border-cyan-500/20">
                <ShieldCheck className="w-6 h-6 sm:w-8 sm:h-8 text-cyan-400" />
              </div>
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-white">Cybersecurity & Staffing</h1>
                <p className="text-xs sm:text-sm text-slate-400">Enterprise-grade template with dark glassmorphism.</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 sm:gap-3 w-full md:w-auto">
               <Button onClick={() => setShowPreview(!showPreview)} variant="outline" className="border-slate-700 dark:border-slate-600 text-slate-300 hover:bg-slate-800 dark:hover:bg-slate-700 hover:text-white flex-1 sm:flex-initial">
                <Eye className="w-4 h-4 mr-2" /> <span className="hidden xs:inline">{showPreview ? 'Hide' : 'Show'} Preview</span>
              </Button>
              <Button onClick={copyToClipboard} variant="outline" className="border-slate-700 dark:border-slate-600 text-slate-300 hover:bg-slate-800 dark:hover:bg-slate-700 hover:text-white flex-1 sm:flex-initial">
                <Copy className="w-4 h-4 mr-2" /> <span className="hidden xs:inline">Copy Code</span>
              </Button>
              <Button onClick={downloadHTML} className="bg-cyan-600 hover:bg-cyan-700 dark:bg-cyan-500 dark:hover:bg-cyan-600 text-white border-0 flex-1 sm:flex-initial">
          ...
          <div className="mt-6 pt-6 border-t border-slate-800 dark:border-slate-700">
            <h3 className="text-xs sm:text-sm font-medium text-slate-400 mb-3">Recommended Subject Lines:</h3>
            <div className="grid md:grid-cols-2 gap-2 sm:gap-3">
              <div className="p-2 sm:p-3 bg-slate-900/50 dark:bg-slate-800/50 rounded border border-slate-800 dark:border-slate-700 text-slate-300 text-xs sm:text-sm font-mono cursor-pointer hover:border-cyan-500/50 dark:hover:border-cyan-400/50 transition-colors" onClick={() => navigator.clipboard.writeText("🛡️ Fortify Your Enterprise: Elite Cybersecurity Staffing Solutions")}>
                🛡️ Fortify Your Enterprise: Elite Cybersecurity Staffing Solutions
              </div>
              <div className="p-2 sm:p-3 bg-slate-900/50 dark:bg-slate-800/50 rounded border border-slate-800 dark:border-slate-700 text-slate-300 text-xs sm:text-sm font-mono cursor-pointer hover:border-cyan-500/50 dark:hover:border-cyan-400/50 transition-colors" onClick={() => navigator.clipboard.writeText("Secure Your Infrastructure with Top 1% Security Talent")}>
          ...
          <div className="mt-6 pt-6 border-t border-slate-800 dark:border-slate-700">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-base sm:text-lg font-semibold text-white">Send Email</h3>
            </div>
            <div className="space-y-4">
              <div>
                <label htmlFor="recipient-email" className="block text-sm font-medium text-slate-300 mb-2">Recipient Email Address</label>
                <input id="recipient-email" type="email" value={recipientEmail} onChange={(e) => setRecipientEmail(e.target.value)} placeholder="recipient@example.com" className="w-full px-4 py-2 bg-slate-900 dark:bg-slate-800 border border-slate-700 dark:border-slate-600 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-cyan-500 dark:focus:ring-cyan-400 focus:border-cyan-500 dark:focus:border-cyan-400 outline-none transition-colors" />
              </div>
              <div>
                <label htmlFor="email-subject" className="block text-sm font-medium text-slate-300 mb-2">Email Subject</label>
                <input id="email-subject" type="text" value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} placeholder="Email subject" className="w-full px-4 py-2 bg-slate-900 dark:bg-slate-800 border border-slate-700 dark:border-slate-600 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-cyan-500 dark:focus:ring-cyan-400 focus:border-cyan-500 dark:focus:border-cyan-400 outline-none transition-colors" />
              </div>
              <Button onClick={sendEmail} disabled={isSending || !recipientEmail} className="w-full bg-cyan-600 hover:bg-cyan-700 dark:bg-cyan-500 dark:hover:bg-cyan-600 text-white gap-2">
        ...
        {/* Live Preview */}
        {showPreview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-xl overflow-hidden border border-slate-800 dark:border-slate-700 bg-black dark:bg-slate-950 shadow-2xl transition-colors">
            <div className="bg-[#020617] dark:bg-slate-950 p-2 sm:p-4 flex justify-center">
              <iframe srcDoc={emailHTML} title="Preview" className="w-full max-w-[720px]" style={{ border: 'none', height: '2400px', minHeight: '600px', background: 'transparent' }} />
            </div>
          </motion.div>
        )}
      </motion.div>
